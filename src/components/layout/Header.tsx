      ...
      <header
        className={cn(
          ...
          overlay
            ...
            : "glass text-ink shadow-soft-sm border-b",
        )}
      >
        <div className="mx-auto flex h-16 w-full max-w-[90rem] items-center gap-2 px-4 sm:h-18 sm:gap-4 sm:px-6 lg:px-10">
          ...
          >
            ...
            <span
              className={cn(
                "transition-colors",
                overlay ? "text-white/70" : "text-accent",
              )}
            >
              Deutschland
            </span>
          ...
          >
            ...
            <kbd className="text-2xs rounded border border-current/20 px-1.5 py-0.5 opacity-70">
              {isMac ? "⌘K" : "Ctrl K"}
            </kbd>
          </button>
          ...
          >
            <BookmarkIcon />
            {tripCount > 0 && (
              <span className="bg-accent text-on-accent absolute top-1.5 right-1.5 flex size-4.5 items-center justify-center rounded-full text-[10px] font-bold">
                {tripCount}
              </span>
            )}
          </Link>
      ...
      {open && (
        ...
        >
          ...
          <motion.nav
            ...
            className="border-line bg-surface shadow-soft-xl absolute inset-y-0 right-0 flex w-[min(22rem,88vw)] flex-col border-l"
          >
            <div className="flex h-16 items-center justify-between px-5 sm:h-18">
              ...
              <button
                ...
                className="hover:bg-surface-2 inline-flex size-11 items-center justify-center rounded-full"
              >
                <CloseIcon />
              </button>
            </div>
              ...
              {navItems.map((item, i) => (
                ...
                >
                  <Link
                    href={item.href}
                    onClick={onClose}
                    className="hover:bg-surface-2 flex min-h-14 items-center justify-between rounded-2xl px-4 font-serif text-xl transition-colors"
                  >
                    {t(item.key)}
                    {item.key === "plan" && tripCount > 0 && (
                      <span className="bg-accent text-2xs text-on-accent flex size-6 items-center justify-center rounded-full font-bold">
                        {tripCount}
                      </span>
                    )}
                  </Link>
